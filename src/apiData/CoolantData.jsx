...
const CoolantData = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("https://tcu-backend-69pu.onrender.com/get_engine_coolant");
    ...
    return (
        <div className="bg-green-500 rounded-xl">
            <LineChart
                xAxis={[{ data: Array.from({ length: coolantData.length }, (_, i) => i + 1) }]}
                series={[
                    {
                        data: coolantData,  color: '#000000'
                    },
                ]}
                width={500}
                height={300}
